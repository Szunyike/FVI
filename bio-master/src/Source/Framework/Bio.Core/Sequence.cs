...
namespace Bio
{
       ...
       public sealed class Sequence : ISequence
    {
        ...
        public ISequence GetSubSequence(long start, long length)
        {
            if (start >= this.Count)
            {
                byte[] sub1Sequence = new byte[0];
                Sequence seq1 = new Sequence { _sequenceData = sub1Sequence, Alphabet = this.Alphabet, ID = this.ID, Count = sub1Sequence.Length };
                if (this._metadata != null)
                    seq1._metadata = new Dictionary<string, object>(this._metadata);
                return seq1;
            }
            if (start < 0)
            {
            ...
            {
                if (index +start < this._sequenceData .Count () )
                {
                    subSequence[index] = this._sequenceData[start + index];
                }
                else
                {
                    subSequence[index] = 78;
                }
        ...
        /// </summary>
        /// <param name="start">The index of the first symbol in the range.</param>
        /// <param name="end">The number of symbols in the range.</param>
        /// <returns>The sub-sequence.</returns>
        public ISequence GetSubSequenceByStartAndEnd(long start, long end)
        {
            if (start >= this.Count) start = this.Count - 1;
                             
            if (start < 0) start = 0;
            if (end  > this.Count ) end = this.Count -1;

            byte[] subSequence = new byte[end -start];
            for (long index = 0; index < end-start; index++)
            {
                subSequence[index] = this._sequenceData[start + index];
            }
