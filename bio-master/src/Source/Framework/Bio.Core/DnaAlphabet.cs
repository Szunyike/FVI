...
namespace Bio
{
    ...
    public class DnaAlphabet : IAlphabet
    {
        ...
        /// </summary>
      public   readonly   Dictionary<byte, byte> nucleotideValueMap = new Dictionary<byte, byte>();
        ...
        /// <returns></returns>
        public string Remove_Non_Alphabetic_Chars(string seq)
        {
            byte[] bytes = System.Text.Encoding.UTF8.GetBytes(seq);
            System.Text.StringBuilder sb = new System.Text.StringBuilder();
            foreach (var value in bytes)
                if (this.nucleotideValueMap.ContainsKey(value))
                {
