...
namespace Bio.IO.BAM
{
    ...
    public partial class BAMParser : IDisposable, ISequenceAlignmentParser
    {
        ...
        /// <returns></returns>
        public static IList<Chunk> GetChunks(BAMReferenceIndexes refIndex)
        {
        ...
        /// <param name="end"></param>
        /// <returns></returns>
        public static IEnumerable <Bio.IO.SAM .SAMAlignedSequence > GetAlignmentMapII(Stream reader, 
              string refSeqName , int refSeqIndex , int start = 0, int end = 10000000 , BAMIndexStorage bamIndexStorage = null)
        {
            if (reader == null || reader.Length == 0)
            {
                throw new Exception(Properties.Resource.BAM_InvalidBAMFile);
           ...
           Stream readStream = reader;
            ValidateReader(readStream);

            BAMParser BP = GetHeaderII(readStream );
             
            {
                refSeqIndex = BP.refSeqNames.IndexOf(refSeqName);
                ...
                if (bamIndexStorage != null)
                {
                    ...
                    foreach (SAMAlignedSequence alignedSeq in GetAlignedSequencesII(chunks, start, end))
                    {
        ...
        private static IEnumerable<SAMAlignedSequence> GetAlignedSequencesII(IList<Chunk> chunks, int start, int end)
        {
        ...
        /// <param name="end"></param>
        /// <returns></returns>
        public IEnumerable<SAMAlignedSequence> GetAlignmentWithIndexYield(BAMIndexStorage bamIndexStorage, int refSeqIndex, int start, int end)
        {
            IList<Chunk> chunks;
            ...
            foreach (SAMAlignedSequence alignedSeq in GetAlignedSequencesIII(chunks, start, end))
            {
                yield return alignedSeq;
            }
        ...
        private static void ValidateReader(Stream readStream)
        {
        ...
        /// Parses the BAM file and returns the Header.
        /// </summary>
        private static BAMParser GetHeaderII(Stream readStream)
        {
            BAMParser BP = new BAM.BAMParser();
            BP.readStream = readStream;
            SAMAlignmentHeader header = new SAMAlignmentHeader();
            BP.refSeqNames = new RegexValidatedStringList(SAMAlignedSequenceHeader.RNameRegxExprPattern);
            BP.refSeqLengths = new List<int>();
            readStream.Seek(0, SeekOrigin.Begin);
            BP.deCompressedStream = null;
            byte[] array = new byte[8];
            BP.ReadUnCompressedData(array, 0, 8);
            int l_text = Helper.GetInt32(array, 4);
            byte[] samHeaderData = new byte[l_text];
            if (l_text != 0)
            {
                BP.ReadUnCompressedData(samHeaderData, 0, l_text);
            }

            BP.ReadUnCompressedData(array, 0, 4);
            int noofRefSeqs = Helper.GetInt32(array, 0);
            ...
            for (int i = 0; i < noofRefSeqs; i++)
            {
                ...
                int len = Helper.GetInt32(array, 0);
                byte[] refName = new byte[len];
                BP.ReadUnCompressedData(refName, 0, len);
                BP.ReadUnCompressedData(array, 0, 4);
                int refLen = Helper.GetInt32(array, 0);
                BP.refSeqNames.Add(Encoding.UTF8.GetString(refName, 0, refName.Length - 1));
                BP.refSeqLengths.Add(refLen);
            }
            ...
            for (int i = 0; i < BP.refSeqNames.Count; i++)
            {
                string refname = BP.refSeqNames[i];
                int length = BP.refSeqLengths[i];
                header.ReferenceSequences.Add(new ReferenceSequenceInfo(refname, length));
            }
        ...
        /// Gets aligned sequence from the specified chunks of the BAM file which overlaps with the specified start and end co-ordinates.
        public IEnumerable <SAMAlignedSequence> GetAlignedSequencesIII(IList<Chunk> chunks, int start, int end)
        {
            List<SAMAlignedSequence> alignedSeqs = new List<SAMAlignedSequence>();
            foreach (Chunk chunk in chunks)
            {
